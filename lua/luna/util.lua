...
---@param amount number 0-1, 1 leaves colors untouched, 0 fully desaturates to grey_light
function M.apply_accent(colors, amount)
  amount = math.max(0, math.min(1, amount))
  if amount == 1 then
    return
  end
  local neutral = colors.grey_light
  for _, key in ipairs({ "keyword", "number", "func", "type", "string", "signal" }) do
    if colors[key] then
      colors[key] = M.blend(colors[key], amount, neutral)
    end
  end
end

---@param groups luna.Highlights
